...
const EventsHub: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-10 animate-fadeIn">
      <div className="bg-gradient-to-r from-brand-900 to-brand-700 rounded-[3rem] p-10 text-white relative overflow-hidden shadow-2xl">
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
          <div className="w-32 h-32 bg-white/10 rounded-full flex items-center justify-center backdrop-blur-md border border-white/20">
             <Moon size={64} className="text-brand-300" fill="currentColor" />
          </div>
          <div className="space-y-4 text-center md:text-left">
            ...
            <div className="flex flex-wrap gap-4 justify-center md:justify-start">
               <div className="bg-white/10 px-4 py-2 rounded-2xl border border-white/20">
                  <p className="text-[10px] font-black uppercase tracking-widest text-brand-300">আজকের হিজরি তারিখ</p>
                  <p className="text-xl font-bold">{hijriDate}</p>
               </div>
               <div className="bg-white/10 px-4 py-2 rounded-2xl border border-white/20">
                  <p className="text-[10px] font-black uppercase tracking-widest text-brand-300">ইংরেজি তারিখ</p>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            {loading ? (
              ...
              events.map((e: any) => (
                <div key={e.id} className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex justify-between items-center group cursor-pointer hover:border-brand-200 transition-all">
                  <div className="space-y-1">
                     <p className="text-[10px] font-black text-brand-600 uppercase">{eventTypeLabels[e.type] || e.type}</p>
                     <h3 className="font-bold text-gray-800 group-hover:text-brand-900">{e.title}</h3>
                  ...
                  <div className="flex flex-col items-end gap-2">
                     <span className="bg-brand-50 text-brand-700 px-3 py-1 rounded-full text-[10px] font-black">আসছে</span>
                     <ChevronRight size={18} className="text-gray-300 group-hover:text-brand-700" />
                  </div>
                </div>
              ))
            )}
...
const ReminderItem = ({ icon, title, time }: { icon: React.ReactNode, title: string, time: string }) => (
  <div className="flex items-center justify-between p-4 hover:bg-gray-50 rounded-2xl transition-all">
    <div className="flex items-center gap-4">
      <div className="p-3 bg-brand-100 text-brand-700 rounded-xl">{icon}</div>
      <p className="font-bold text-gray-800">{title}</p>
    </div>
    <p className="text-lg font-black text-brand-900">{time}</p>
  </div>
);
